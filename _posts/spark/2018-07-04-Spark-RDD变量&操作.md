...
```  
/**
  * zeroValue: 表示每个分组的初始基准值
  *
  * seqOp: (U, T) => U
  *       每个分区内不同分组的元素聚合 ( 类似Map.Shuffle.Combine )
  *       U:当前基准值,T:当前元素 将分组内元素依次传入执行函数,结果写入基准值.
  *
  * combOp: (U, U) => U
  *       分区间相同分组的聚合,将各分区执行seqOp函数后的基准值结果进行聚合 (类似Reduce)
  *
  */
val rdd =sc.parallelize(1 to 10).union(sc.parallelize(4 to 8)).map(x=>(x,1))
val aggregateResult = rdd.aggregateByKey(0)(
  seqOp = (sv, tv) =>  sv + tv,  //每个分区的同分组(同单词),计数累加
  combOp = (v1, v2) => v1 + v2 //不同分区的同组(同单词),计数累加
)
val reault = aggregateResult.sortBy(x=>x._1).collect()

reault.map(x=>print(s"${x}\t"))
//输出结果: (1,1)	(2,1)	(3,1)	(4,2)	(5,2)	(6,2)	(7,2)	(8,2)	(9,1)	(10,1)
```
