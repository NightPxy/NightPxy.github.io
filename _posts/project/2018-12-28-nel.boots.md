...
**主(单)线程的Kafka连接** 减少物理连接和实际消费实例数量,利于横向扩展   
**自动Offset管理** 基于消费实现方法是否成功返回true   
**自动Kafka分区感知&分区有序** 自动根据Kafka分区平衡实际的消息线程数  
**自动消费限速**  
...
* 发送消息封装(只关注业务部分而屏蔽发送实现细节)  
* 消费线程池封装(只关注业务部分而屏蔽消费实现细节)  
**主(单)线程的MQ连接&多线程处理** 单物理连接,利于横向扩展   
...
val df = spark.read
  ...
  .option("spark.cus.hbase.start_key","10000000")
  .load()
```
...
df.write
  ...
  .option("spark.cus.hbase.url","192.168.198.151")
  .option("spark.cus.hbase.table","dbo:test")
  .option("spark.cus.hbase.tmp_path","hdfs://192.168.198.151:9000/xx/tmp/hbase_xxx")
